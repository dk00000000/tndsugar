    ...
    public function postAlltransactionData($data)
	{
		...
		$json_data=$data;*/		
	        ...
	        for ($i=0; $i < sizeof($data) ; $i++)
	            ...
	            $insertQry = $qry->fetchQuery($filepath,'Q001','INSERT_FREGTRAN',$oldFilter,$newFilter);
	            $stid = oci_parse($this->conn, $insertQry);
	            $res=oci_execute($stid); 
	            $affected_rows = oci_num_rows($stid);
	            $r = oci_commit($this->conn);
	            //IF RECORD INSERTED SUCCESSFULY

	            
	            $total_data=array();
	            if($affected_rows > 0){
	            	 //echo "Query".$insertQry."Affected rows".$affected_rows;
	            	 $postresponseQry = $qry->fetchQuery($filepath,'Q001','INSERT_FREGRESPONSE',$oldFilter,$newFilter);
...
$obj = new Postapiinput();
$data=$_REQUEST['data'];
$obj ->postAlltransactionData($data);

//http://203.127.5.7/tndsugar/api/Postapiinput.php?username=administrator&password=adm
