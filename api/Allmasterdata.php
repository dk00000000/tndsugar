    ...
    public function getAllmasterData($data)
	{
        ...
        foreach ($data['AllMasters'][0] as $key => $value) 
        {
			...
			if($value==''){
				...
				$date =explode(" ", $value); //'01/01/0001';//$value;
				$sync_date = $date[0];
			}

			$oldFilter = array(':PSYNC_DATE');
			$newFilter = array($sync_date);
			$newSectonQry = $qry->fetchQuery($filepath,'Q001',$key,$oldFilter,$newFilter);
			//echo "Qry :".$newSectonQry;
		    ...
		    $fregDelQry = $qry->fetchQuery($filepath,'Q001','FREG_DELTED',$oldFilter,$newFilter);
			$fregDelRes = $dsbObj->getData($fregDelQry);
			$result['FREG_DELETED']['DELETED']=$fregDelRes;

			$hsDelQry = $qry->fetchQuery($filepath,'Q001','HS_DELTED',$oldFilter,$newFilter);
			$hsDelRes = $dsbObj->getData($hsDelQry);
			$result['HS_DELETED']['DELETED']=$hsDelRes;

			$fsvDelQry = $qry->fetchQuery($filepath,'Q001','FSV_DELTED',$oldFilter,$newFilter);
			$fsvDelRes = $dsbObj->getData($fsvDelQry);
			$result['FSV_DELETED']['DELETED']=$fsvDelRes;
...
/*
http://203.127.5.7/tndsugar/api/Allmasterdata.php?username=kadamkg&password=k&data={"AllMasters":[
{
"SECMAST":"",
"BACK_DATA":"",
"CANEDTMAST":"",
