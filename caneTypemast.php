...
$lgs->lg->trace("--START CANETMAST FORM--");
...
if($action == 'update' || $action == 'view')
{
	...
	$mainRes = $curd->GetSelData($oldfilter, $newfilter, 'CANETMAST.ini', 'GETUPDDATA');
	$lgs->lg->trace("--CANETMAST RESULT OF SELECTED RECORD--".json_encode($mainRes));
	//print_r($mainRes);
}

$oldLovFilter = array(':PCOMP_CODE', ':PTBLNM');
$newLovFilter = array($_SESSION['COMP_CODE'],'CANETMAST');
$ctCodeRes = $dsbObj->getLovQry(4,$oldLovFilter,$newLovFilter);
$lgs->lg->trace("--ctCodeRes--".json_encode($ctCodeRes));
//To get Cane Group LOV from dashbord's getLov function
$CGLOVres = $dsbObj->getLovQry(48,$oldLovFilter,$newLovFilter);
...
if(action_flag == 'view')
{
	...
	$("#txtEnglish").prop("readonly", true); 
	$("#txtMarathi").prop("readonly", true); 
	$("#ctgrp").prop("disabled", true);
...
$("#btn_submit").click(function(event){
	...
	{  
		$.ajax({
              url: "caneTypemastserver.php",
              //data:$('#form_fgmast').serialize()+'&'+$.param({'action':action_flag, oldfilter:oldfilterArr}),
			  data:$('#form_canetmast').serialize()+'&'+$.param({'action':action_flag}),
...
<div class="">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="x_panel">
				<div class="x_content">
				<form id="form_canetmast" name="form_canetmast" class="form-horizontal form-label-left" onsubmit="return false;">
				<ul class="contactus-list">
				<span class="section"><?=$rfObj->readData('CNTMST',$langPath); ?></span>
					...
					<div class="panel-body">
						...
						<div class="form-group">
							...
							<div class="col-md-2 col-sm-2 col-xs-4">
								<input type="text" id="ct_code" name="ct_code" class="form-control col-md-7 col-xs-12" placeholder="Enter Code" value="<?php if($action == 'add')
												 {	echo $ctCodeRes[0]['CODE']; }
												 else 
												 {	echo $mainRes[0]['CT_CODE']; } ?>">
							</div>
						</div><!--//form-group-->
						...
						<div class="form-group">
							...
							<li><div class="col-md-3 col-sm-3 col-xs-6">
								<input type="text" id="txtEnglish" name="ct_name" class="form-control col-md-7 col-xs-12 txtEnglish" placeholder="Enter Name in English" valid="required" errmsg="Please Enter Name." value="<?php echo $mainRes[0]['CT_NAME']; ?>">
							</div></li>
						</div><!--//form-group-->
						...
						<div class="form-group">
							...
							<li><div class="col-md-3 col-sm-3 col-xs-6">
								<input type="text" id="txtMarathi" name="ct_mname" class="form-control col-md-7 col-xs-12 txtMarathi" placeholder="Enter Name in Marathi" valid="required" errmsg="Please Enter Name." value="<?php echo $mainRes[0]['CT_MNAME']; ?>">
							</div></li>
						</div><!--//form-group-->
						
						<div class="form-group" id="ctgrp_div">
							<label class="control-label col-md-3 col-sm-3 col-xs-12"><?=$rfObj->readData('GRP',$langPath); ?></label>
							<li><div class="col-md-3 col-sm-3 col-xs-6">
								<select id ="ctgrp" name="ctgrp" class="form-control col-md-7 col-xs-12" valid="required" errmsg="Please Select Group.">
								<option value="">Select</option>
								<? for($i=0;$i<sizeof($CGLOVres);$i++)
								{
								?>
								<option value="<?=$CGLOVres[$i]['CGP_CODE']?>" <? if($mainRes[0]['CT_GROUP'] == $CGLOVres[$i]['CGP_CODE']) {?> selected="selected" <? }?>><?=$CGLOVres[$i]['CGP_NAME']?></option>
								<? 
								}
								?>	
								</select>
