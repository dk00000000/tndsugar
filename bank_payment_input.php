<?  
  ...
  $billTypeRes = $dsbObj->getLovQry(73,$oldLovFilter,$newLovFilter);
  $titlename = $dsbObj->getLovQry(138,$oldLovFilter,$newLovFilter);

  $menunameRes = $curd->GetSelData($oldLovFilter,$newLovFilter,'bank_payment.ini','MENUNAME');
     ...
     <div class="clearfix"></div>
      ...
      <div class="row">
        ...
        <div class="x_panel">
          ...
          <div class="x_content">
           <form id="bankpayform" class="form-horizontal form-label-left" method="POST" action="bank_payment.php" target="_blank">
          ...
          <div class="panel-heading" id="addpanel">Input</div>
            ...
            <div class="form-group">
              <label class="control-label col-md-3 col-sm-3 col-xs-12" >Contract Type</span></label>
              <div class="col-md-6 col-sm-6 col-xs-12">
                <select id ="contract_type" name="contract_type" class="form-control col-md-7 col-xs-12">
                  <option value="">Select</option>
                <? for($i=0;$i<sizeof($contractTypeRes);$i++){?>
                  <option value="<?=$contractTypeRes[$i]['CT_CODE'].'||'.$contractTypeRes[$i]['CT_MNAME']?>"><?=$contractTypeRes[$i]['CT_CODE'].' || '.$contractTypeRes[$i]['CT_MNAME']?></option>
                <? } ?> 
                </select>
              </div>
            </div>      
            ...
            <div class="ln_solid"></div>
              ...
              <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-4">
                ...
                <input type="button" name="export" class="btn btn-success" id="btn_export" value="Export to Excel">
              </div>
              </div>  
...
<? include("footer.php");?>
 ...
 $(document).ready(function() {
   $('#btn_export').on('click', function() {
      ...
      var menu_name = '<?php echo $menuname;?>';
      var season = $('#season').val();
      var contract_type = $('#contract_type').val();
      var fornight = $('#fortnight').val();
      var bill_type = $('#bill_type').val();

      window.open('bankpayment_excel.php?menu_name='+menu_name+'&season='+season+'&fortnight='+fornight+'&contract_type='+contract_type+'&bill_type='+bill_type,'_blank');
  });
