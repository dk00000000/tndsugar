     ...
     <div class="clearfix"></div>
      ...
      <div class="row">
        ...
        <div class="x_panel">
          ...
          <div class="x_content">
           <form id="districtform" class="form-horizontal form-label-left" method="POST" action=" bankbranch_vlgfarmer.php" target="_blank">
          ...
          <div class="panel-heading" id="addpanel">Input</div>
            ...
            <div class="form-group">
              <label class="control-label col-md-3 col-sm-3 col-xs-12" >Bank Branch</span></label>
              <div class="col-md-6 col-sm-6 col-xs-12">
                <select id ="bankbranch" name="bankbranch[]" class="form-control col-md-7 col-xs-12" multiple="multiple">
                  <option value="">Select</option>
                <? for($i=0;$i<sizeof($bankbranchres);$i++){?>
                  <option value="<?=$bankbranchres[$i]['BR_CODE']?>"><?=$bankbranchres[$i]['BR_NAME']?></option>
                <? } ?> 
                </select>
              </div>
            </div>     
...
$('#fornight').empty();
	...
	$('#bill_type').on('change',function(){
		...
		$('#fornight').empty();
		 ...
		 $.ajax(
				{
					url: "bankbranch_vlgfarmer_input.php",
            ...
            /*alert(response);*/
						...
						{
							$('#fornight').append($('<option>').text(value['FORTNIGHT']).attr('value', value['SND_FNNO']+'-'+value['FORTNIGHT']));						
						});
					}
