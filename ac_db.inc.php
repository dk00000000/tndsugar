...
//		protected $conn = null; // originally it's protected, modified by Padmraj on 28 Oct. 2013
		...
		*/
		...
		{
			//echo "Username ".$_SESSION['USER']."Password". $_SESSION['IDTTY'];
			$logs = new Logs();  			
			...
			//echo"<br>------------------in execute function ".$this->conn.'*****'.$sql;
			$this->stid = oci_parse($this->conn, $sql);
			//echo"<br>------------------in execute function ".$this->stid;
			/*if($this->stid)
			{
		...
		*/
		public function execCommit($sql, $action, $bindvars = array()) 
		{
			...
			$logs->lg->trace("ANKUSH CONNECTION CODE EXECUTE COMMIT "); 
			...
			//$affected_rows =oci_num_rows($this->stid);
			//$logs->lg->trace("affected_rows Ankush ".$affected_rows);

			return $result;
		} // end of execute function


		public function execCommitgetLastInsertedID($sql, $action, $bindvars = array()) 
		{
			$logs = new Logs();
            ...
            oci_bind_by_name($this->stid, ':id', $id, 5000);  
			...
			return $id."-".$r;
		} // end of execute Return Last Inserted Id function
		...
		public function execFetchAll($sql, $action, $bindvars = array())
		 {
			$logs = new Logs();  
			$res=array();	
			$logs->lg->trace("IN execFetchAll".$sql);
			//echo $sql;

			if((isset($_SESSION['USER'])) && (isset($_SESSION['IDTTY'])))
			{
				$logs->lg->trace("IN GET DATA USERCODE".$_SESSION['USER']."PASSWORD".$_SESSION['IDTTY']);
		...
		*/
		...
		public function executeProc($sql, $action, $bindvars) 
		{
			...
			$logs->lg->trace("--Session User--".$_SESSION['USER']);	
			/*
				Below condition use to execute procedure without user login
				added by Amit Dubey dated on 10-05-2018 for 
				Production Sales Stock(PSS)	
			*/
			if($_SESSION['USER']!='N'){
				$logs->lg->trace("--With session login--".$_SESSION['USER']);	
				$this->conn = @oci_pconnect($_SESSION['USER'], $_SESSION['IDTTY'], DATABASE, AL32UTF8);
				$this->stid = oci_parse($this->conn, $sql);	
			}else{
				$logs->lg->trace("--Without session login--".$_SESSION['USER']);	
				$this->conn = @oci_pconnect("NIS", "NIS", DATABASE, AL32UTF8); 
				$this->stid = oci_parse($this->conn, $sql);	
			}
